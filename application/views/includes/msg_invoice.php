<div class="row wrpr_singl_msg" id="msg_wapr_<?php echo $value['msg_id'];?>" data-id1="<?php echo $value['msg_id'];?>">
    <?php 
    $job_escrow_amt = $this->Common->job_escrow_amt($value['job_id'],$value['date']);
    ?>
    <div id="outer_Wrpr_<?php echo $value['msg_id'];?>" data-id1="<?php echo $value['msg_id'];?>">
            ...
            <div class="row">
                <div class="col-xs-12 propmain_heading">
                    <p>
                        <b>
                            <?php 
                                echo date("h:i: a, d M Y",strtotime($this->Common->gettimeinmyzone($value['date'])));
                            ?>
                        </b>
        ...
        <?php if( ($value['is_seen'] == 1) && $value['send_id']==$myuser['u_id']){ ?>
         ...
         echo "Seen ";
         echo $this->check->timeAgo(strtotime(date_format($dt_obj, 'Y-m-d H:i:s')));  ?></span><?php }else{
         if($value['marked_as_complete']  == 'yes'){
         ?>
         
         <span class="label label-success pull-right"> Seller Marked as completed</span>
        ...
        <?php }} echo "</div>";
            ...
            <div class="row proposa_back">
                ...
                <div class="col-md-2 col-sm-12 col-xs-12">
                    <div class="button_action" id="wrpr_invc_section_<?php echo $value['msg_id'];?>">
                        ...
                        ?>
                                    <button type="button" class="btn btn_inovce cancelinvoice" <?php echo $disabled_action;?> data-value="<?php echo $value['msg_id']; ?>">Cancel</button>
                                <?php }else{ ?>
                                    <a href="javascript:void(0)" type="button" class="btn btn-success btn_inovce accept_invoice" <?php echo $disabled_action;?> data-value="<?php echo $value['msg_id']; ?>">Accept</a>
                                    <button type="button" class="btn btn-danger btn_inovce reject_invoice" <?php echo $disabled_action;?> data-value="<?php echo $value['msg_id']; ?>">Reject</button>
                                <?php } ?>
            ...
            <?php  
                    if($value['custom_status_extent']=="6"){
                        $is_dispute_araised = $this->db->query("select * from disputes where msg_id='".$value['msg_id']."' and type='job'");
            ...
            ?>
                            ...
                            ?>    
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-10" style="border-left: 3px solid #cc2424;margin-left: 12px;">
                                        <div class="accept_cont">
                                            <h4>Amount has deposited to <?php echo $winner_name; ?>.</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-10" style="border-left: 3px solid #cc2424;">
                                            <div class="accept_cont">
                                                ...
                                                <h4>Your invoice has been rejected. If you want to raise a dispute <a class="dispute_link" data-href="Disputespage/index/<?php echo $value['job_id'].'/'.$value['msg_id'];?>" href="<?php echo SURL.'Disputespage/index/'.$value['job_id'].'/'.$value['msg_id'];?>" target="_blank"  class="dropdown-toggle chat_btnss2 dispute_link"  data-href="Disputespage/services/<?php echo $value['service_p_id'];?>" >click here</a>.</h4>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="accept_wpar">
        <div class="row">
            ...
            <div class="col-sm-10" style="border-left: 3px solid #007d3d;">
                <div class="accept_cont">
                    <h4>
                        <?php echo date("d M Y",strtotime($this->Common->gettimeinmyzone($value['date']))); ?>
                    </h4>
                    <h4>Your Invoice has been approved. £<?php echo $value['invc_amt']; ?> is deposited to your account</h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <?php 
        if($value['custom_status_extent']=="5"){
            ...
            if($myuser['u_id']==$job_detail['u_id']){ 
                if($value['is_freelancer_rated']=="No"){
                    if($this->uri->segment(2)!="viewChat"){
    ?>
                    <form class="feedback_warp" action="<?php echo SURL.'Chat/giverating';?>" method="post">
                        <h3>Rating</h3>
                        ...
                        <div class="send_feed_left send_content">
                            <div class="feedback_send bg-white">
                                ...
                                <input type="hidden" name="jobid" value="<?php echo $value['job_id']; ?>"/>
                                ...
                                <div class="send_rating ratting_star" >
                                    ...
                                    <button type="submit" class="btn btn-primary submit_rate giveratingg">Submit</button>
                                </div>
                            </div>
                        ...
                        <div class="send_feed_left send_content">
                            <div class="feedback_send bg-white">
                                ...
                                <div class="send_rating ratting_star" >
                                    ...
                                    <button type="submit" class="btn btn-primary submit_rate">Submit</button>
                                </div>
                            </div>
    ...
    <?php 
        if($value['is_freelancer_rated']=="Yes" && $value['is_buyer_rated']=="Yes"){
            ...
            $feedback = $this->db->query("select user_rating.*,users.* from user_rating inner join users on users.u_id=who_rated where msg_id='".$value['msg_id']."' and is_reply='No'")->result_array();
            foreach($feedback as $key=>$feedbackvalue){
    ?>
    <div class="feedback_warp">
        
        <?php 
        if($feedbackvalue['user_status']=="User"){
        ...
        <?php 
            $rating_reply = $this->db->query("select user_rating.*,users.* from user_rating inner join users on users.u_id=who_rated where reply_of='".$feedbackvalue['rating_id']."'")->result_array();
        ...
        <form class="send_feed_right send_content" action="<?php echo SURL.'Chat/replyrating';?>" method="post">
            <div class="feedback_send bg-white">
            ...
            <div class="send_rating ratting_star" >
                ...
                <button type="submit" class="btn submit_rate">Submit</button>
            </div>
        </form>
    ...
    if($job_detail['u_id']==$myuser['u_id'] && $value['custom_status_extent']=="5"){?>
    ...
    <?php if($value['tip_amt']=="0"){?>
        ...
        <!--    <form action="<?php echo SURL.'Chat/givetip'?>" method="post" class="row tip_Wrpr" id="choosetipamt_<?php echo $value['msg_id'];?>">-->
        ...
        <!--                <input type="hidden" value="<?php echo $job_detail['job_slug'];?>" name="job_slug"/>-->
                    ...
                    <div class="col-sm-12 invoice_data">
                        <p>
                            <?php 
                               ...
                               $buyer = $this->db->query('select * from users where u_id = '.$job_detail['u_id'])->result_array()[0];
                               
                                if($job_detail['u_id']==$myuser['u_id']){
                                    echo $buyer['f_name']." ".$buyer['l_name']." has given the tip of £".$value['tip_amt'];
                                }else{
                                    echo "Congratulations! You received ".$value['tip_amt']." tip from ".$buyer['f_name']." ".$buyer['l_name'].". Keep up with the Good work!";
                                }
                                echo "";
                            ?>    
                        </p>
