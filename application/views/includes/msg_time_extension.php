...
<div class="proposal_invoice bg-white" data-id1="<?php echo $value['msg_id'];?>"> 
        ...
        //echo $this->check->timeAgo(strtotime(date_format($dt_obj, 'Y-m-d H:i:s')));?></b> <?php if( ($value['is_seen'] == 1) && $value['send_id']==$myuser['u_id']){ ?>
         ...
         echo "Seen ";
         echo $this->check->timeAgo(strtotime(date_format($dt_obj, 'Y-m-d H:i:s')));  ?></span><?php }?></p> 
    	...
    	<table class="table table-bordered invoicee_detailse">
    		...
    		<tbody>
    			...
    			<tr>
                    ...
                    ?>
                                <button type="button" class="btn btn_inovce cancel_time" <?php echo $disabled_action;?> data-value="<?php echo $value['msg_id']; ?>">Cancel Request</button>
                            <?php }else{?>
                            <div>
                                <button type="button" class="btn btn_inovce accepttime" <?php echo $disabled_action;?> style="background:#007d3d;color:white;" data-value="<?php echo $value['msg_id']; ?>">Accept</button>
                                <button type="button" class="btn btn_inovce reject_time btn-danger" <?php echo $disabled_action;?> data-value="<?php echo $value['msg_id']; ?>">Reject</button>
                            </div>
                            <?php } ?>
