...
class Moderatorlogin extends CI_Controller {
	...
	{ 
		if($this->session->userdata('AdminLoggedIn')){
			redirect(SURL."/ModeratorDashboard");
		}
		...
		if(isset($_POST['submit'])){
			...
			$sql = "select * from users where email='$email' and pass='$pass' and status='Active' and 
			 	user_status in ('Admin','Moderator')";
			 ...
			 if($query->num_rows() > 0){
				...
				$this->session->set_userdata('moderator',TRUE);
			 	redirect(SURL."ModeratorDashboard");
		...
		$this->load->view("Moderatorlogin", $data);
